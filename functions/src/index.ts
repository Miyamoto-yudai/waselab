...
export const sendEvaluationNotification = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      const targetUserId = experiment?.creatorId;
      ...
      const evaluatorName = evaluatorDoc.exists
        ? evaluatorDoc.data()?.name || "参加者"
        : "参加者";

      const pointsAwarded = evaluation.isGood ? (evaluation.pointsAwarded || 1) : 0;
      const message = evaluation.isGood
        ? `${evaluatorName}さんから「${experiment.title}」に対してGood評価を受けました${
            pointsAwarded > 0 ? ` +${pointsAwarded} ポイント獲得` : ""
          }`
...
export const sendReservationNotification = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      const experiment = experimentDoc.data();
      const creatorId = experiment?.creatorId;
...
export const sendCancellationNotification = functions.firestore
  .document("reservations/{reservationId}")
...
export const sendExperimentCompletedNotification = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.status !== "completed" && after.status === "completed") {
      try {
        ...
        console.log(
          `Experiment completed notification created for user ${creatorId}`
        );
        return null;
      } catch (error) {
