...
class AppUser {
  ...
  factory AppUser.fromFirestore(DocumentSnapshot doc) {
    ...
    return AppUser(
      ...
      bio: data['bio'],
      participatedExperiments: (data['participatedExperiments'] ?? 0).toInt(),
      scheduledExperiments: (data['scheduledExperiments'] ?? 0).toInt(),
      completedExperimentIds: List<String>.from(data['completedExperimentIds'] ?? []),
      department: data['department'],
      grade: data['grade'],
      goodCount: (data['goodCount'] ?? 0).toInt(),
      badCount: (data['badCount'] ?? 0).toInt(),
      totalEarnings: (data['totalEarnings'] ?? 0).toInt(),
      monthlyEarnings: (data['monthlyEarnings'] ?? 0).toInt(),
      ...
      gender: data['gender'],
      age: data['age'] != null ? (data['age']).toInt() : null,
      points: (data['points'] ?? data['goodCount'] ?? 0).toInt(), // goodCountをデフォルトポイントとして使用
